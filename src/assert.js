...
const arr = exports.arr = (raw, name, length, _type, _default) => units => {
    ...
    return raw.map(val => {
        ...
        if (_type == 'number') {
            let num
            try { num = mathnum(val)(units) } catch (err) {}
            assert(typeof num == 'number', `Field "${name}" should contain ${_type}s!`)
            return num
        }
        assert(type(val)(units) == _type, `Field "${name}" should contain ${_type}s!`)
        return val
    })
}
